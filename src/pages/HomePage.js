...
const socialLinks = [
  ...
  { 
    name: 'Instagram', 
    url: 'https://www.instagram.com/pakev_official', 
...
function HomePage() {
  ...
  const BannerBox = ({ slot, fallbackColor, className }) => {
    ...
    const sharedClasses = `relative overflow-hidden rounded-2xl group block ${className} ${!imgUrl ? fallbackColor : ''} ${!isLink ? 'cursor-default' : ''}`;
  ...
  const ProductCard = ({ product }) => (
    <Link to={`/product/${product.id}`} className="flex-none w-64 md:w-72 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group overflow-hidden">
      <div className="h-48 relative bg-gray-50 rounded-t-xl overflow-hidden p-4 flex items-center justify-center">
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
          ...
          <span className={`text-[10px] font-bold px-2 py-1 rounded shadow-sm border ${product.codAvailable ? 'bg-green-100 text-green-700 border-green-200' : 'bg-blue-100 text-blue-700 border-blue-200'}`}>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <h3 className="font-bold text-gray-900 mb-2 line-clamp-2 text-sm md:text-base group-hover:text-orange-600 transition-colors">{product.title}</h3>
    ...
    return (
      <div className="mb-12">
        <div className="flex items-center justify-between mb-6 px-1">
          ...
          <Link to={`/section/${section.name.toLowerCase().replace(/\s+/g, '-')}`} className="flex items-center text-orange-600 font-semibold hover:text-orange-700">
        ...
        {section.isMarquee ? (
          ...
          <div className="relative group">
            <button onClick={() => scroll('left')} className="absolute left-0 top-1/2 -translate-y-1/2 -ml-4 z-10 w-10 h-10 bg-white rounded-full shadow-lg items-center justify-center text-gray-800 opacity-0 group-hover:opacity-100 transition-opacity hidden md:flex">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </button>
            <div ref={scrollRef} className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x" style={{ scrollbarWidth: 'none' }}>
            ...
            <button onClick={() => scroll('right')} className="absolute right-0 top-1/2 -translate-y-1/2 -mr-4 z-10 w-10 h-10 bg-white rounded-full shadow-lg items-center justify-center text-gray-800 opacity-0 group-hover:opacity-100 transition-opacity hidden md:flex">
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <section className="grid grid-cols-3 gap-2 md:gap-4 mb-16">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-white rounded-xl md:rounded-2xl shadow-sm border border-gray-100 p-3 md:p-6 flex flex-col items-center justify-center text-center">
              <p className="text-xl md:text-3xl font-bold text-gray-900">{stat.value}</p>
              <p className="mt-1 md:mt-2 text-[10px] md:text-xs uppercase tracking-widest text-gray-500 font-semibold leading-tight">{stat.label}</p>
            </div>
          ))}
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
              <div className="space-y-6">
                ...
                <p className="text-gray-500 text-sm leading-relaxed max-w-xs">
                  We are a proud Pakistani brand dedicated to the EV sector under the leadership of our CEO,<b> Muhammad Haroon.</b>
                </p>
                <p className="text-gray-500 text-sm">Address: Back of ZTBL head rajkan, yazman, bahawalpur</p>
              </div>

              <div>
                <h4 className="text-xs font-bold text-orange-600 uppercase tracking-widest mb-8">Quick Links</h4>
                <ul className="space-y-4">
                  {['About Us', 'Branches', 'EV Calculator', 'Load Calculator'].map((label) => (
                    <li key={label}>
                      <Link to={`/${label.toLowerCase().replace(/\s+/g, '-')}`} className="text-gray-600 hover:text-orange-600 font-medium transition-colors text-sm flex items-center gap-2 group">
                        <span className="w-1.5 h-1.5 rounded-full bg-transparent group-hover:bg-orange-600"></span>
                        {label}
                      </Link>
                    </li>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {socialLinks.map((social) => (
                    <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className={`w-11 h-11 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 transition-all hover:text-white hover:scale-110 hover:shadow-lg ${social.hover}`}>
                      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d={social.icon} /></svg>
                    </a>
                  ))}
                </div>
              </div>
