...
export default function SectionProducts() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      const readableSlug = sectionSlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      ...
      try {
        ...
        const filteredProducts = allProducts.filter(product => {
          ...
          const secName = product.section?.name || '';
          const slug = secName.toLowerCase().replace(/\s+/g, '-');
          
          if (slug === sectionSlug) {
            if (!foundName) foundName = secName;
            return true;
          }
          return false;
        });
        ...
        setSectionName(foundName || readableSlug);

      } catch (error) {
        console.error('Error fetching products:', error);
        setSectionName(readableSlug);
      } finally {
        setLoading(false);
      }
    };
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        <div className="mb-8">
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            ...
            <span>/</span>
            <span className="text-gray-900 font-semibold">{sectionName}</span>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl p-12 text-center border border-gray-100">
            ...
            <p className="text-gray-500 mb-6">
                We couldn't find any products in the <strong>{sectionName}</strong> section.
            </p>
